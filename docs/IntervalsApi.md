...
> IntervalList listIntervals(studyKey, opts)
...
let apiInstance = new MednetEdcApi.IntervalsApi();
...
apiInstance.listIntervals(studyKey, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {

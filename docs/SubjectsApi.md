...
> <SubjectList> list_subjects(study_key, opts)
...
begin
  # List subjects in a study
  result = api_instance.list_subjects(study_key, opts)
  p result
rescue OpenapiClient::ApiError => e
  puts "Error when calling SubjectsApi->list_subjects: #{e}"
end
```

#### Using the list_subjects_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<SubjectList>, Integer, Hash)> list_subjects_with_http_info(study_key, opts)

```ruby
begin
  # List subjects in a study
  data, status_code, headers = api_instance.list_subjects_with_http_info(study_key, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <SubjectList>
rescue OpenapiClient::ApiError => e
  puts "Error when calling SubjectsApi->list_subjects_with_http_info: #{e}"
end
```

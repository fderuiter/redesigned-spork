...
> RecordRevisionList ListRecordRevisions(ctx, studyKey).Page(page).Size(size).Sort(sort).Filter(filter).Execute()
...
func main() {
	...
	resp, r, err := apiClient.RecordRevisionsAPI.ListRecordRevisions(context.Background(), studyKey).Page(page).Size(size).Sort(sort).Filter(filter).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `RecordRevisionsAPI.ListRecordRevisions``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `ListRecordRevisions`: RecordRevisionList
	fmt.Fprintf(os.Stdout, "Response from `RecordRevisionsAPI.ListRecordRevisions`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiListRecordRevisionsRequest struct via the builder pattern

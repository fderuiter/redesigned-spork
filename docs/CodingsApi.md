...
> CodingList listCodings(studyKey, opts)
...
let apiInstance = new MednetEdcApi.CodingsApi();
...
apiInstance.listCodings(studyKey, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {

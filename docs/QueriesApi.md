...
> QueryList listQueries(studyKey, opts)
...
let apiInstance = new MednetEdcApi.QueriesApi();
...
apiInstance.listQueries(studyKey, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {

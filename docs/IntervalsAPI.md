...
> IntervalList ListIntervals(ctx, studyKey).Page(page).Size(size).Sort(sort).Filter(filter).Execute()
...
func main() {
	...
	resp, r, err := apiClient.IntervalsAPI.ListIntervals(context.Background(), studyKey).Page(page).Size(size).Sort(sort).Filter(filter).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `IntervalsAPI.ListIntervals``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `ListIntervals`: IntervalList
	fmt.Fprintf(os.Stdout, "Response from `IntervalsAPI.ListIntervals`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiListIntervalsRequest struct via the builder pattern

...
> StudyList ListStudies(ctx).Page(page).Size(size).Sort(sort).Filter(filter).Execute()
...
func main() {
	...
	resp, r, err := apiClient.StudiesAPI.ListStudies(context.Background()).Page(page).Size(size).Sort(sort).Filter(filter).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `StudiesAPI.ListStudies``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `ListStudies`: StudyList

...
> CodingList ListCodings(ctx, studyKey).Page(page).Size(size).Sort(sort).Filter(filter).Execute()
...
func main() {
	...
	resp, r, err := apiClient.CodingsAPI.ListCodings(context.Background(), studyKey).Page(page).Size(size).Sort(sort).Filter(filter).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `CodingsAPI.ListCodings``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `ListCodings`: CodingList
	fmt.Fprintf(os.Stdout, "Response from `CodingsAPI.ListCodings`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiListCodingsRequest struct via the builder pattern

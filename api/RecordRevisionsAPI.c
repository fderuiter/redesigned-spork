...
RecordRevisionsAPI_listRecordRevisions(apiClient_t *apiClient, char *studyKey, int *page, int *size, char *sort, char *filter)
{
    ...
    // create the path
    char *localVarPath = strdup("/studies/{studyKey}/recordRevisions");
    ...
    record_revision_list_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *RecordRevisionsAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = record_revision_list_parseFromJSON(RecordRevisionsAPIlocalVarJSON);
        cJSON_Delete(RecordRevisionsAPIlocalVarJSON);

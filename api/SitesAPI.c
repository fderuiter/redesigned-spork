...
//
site_list_t*
SitesAPI_listSites(apiClient_t *apiClient, char *studyKey, int *page, int *size, char *sort, char *filter)
{
    ...
    // create the path
    char *localVarPath = strdup("/studies/{studyKey}/sites");
    ...
    site_list_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *SitesAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = site_list_parseFromJSON(SitesAPIlocalVarJSON);
        cJSON_Delete(SitesAPIlocalVarJSON);

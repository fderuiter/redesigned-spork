...
UserList <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new UserList class.
    #'
    ...
    #'
    #' @return UserList as a base R list.
    #' @examples
    #' # convert array of UserList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert UserList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      UserListObject <- list()
      if (!is.null(self$`metadata`)) {
        UserListObject[["metadata"]] <-
          self$`metadata`$toSimpleType()
      }
      if (!is.null(self$`pagination`)) {
        UserListObject[["pagination"]] <-
    ...
    #' @description
    #' Deserialize JSON string into an instance of UserList
    #'
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`data`)) {
        self$`data` <- ApiClient$new()$deserializeObj(this_object$`data`, "array[User]", loadNamespace("openapi"))
      }
      self
    },
    ...
    #' @return UserList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to UserList and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of UserList
    toString = function() {
      self$toJSONString()
    },
...
# UserList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })

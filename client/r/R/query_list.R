...
QueryList <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new QueryList class.
    #'
    ...
    #'
    #' @return QueryList as a base R list.
    #' @examples
    #' # convert array of QueryList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert QueryList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      QueryListObject <- list()
      if (!is.null(self$`metadata`)) {
        QueryListObject[["metadata"]] <-
          self$`metadata`$toSimpleType()
      }
      if (!is.null(self$`pagination`)) {
        QueryListObject[["pagination"]] <-
          self$`pagination`$toSimpleType()
      }
      if (!is.null(self$`data`)) {
        QueryListObject[["data"]] <-
    ...
    #' @description
    #' Deserialize JSON string into an instance of QueryList
    #'
    #' @param input_json the JSON input
    #' @return the instance of QueryList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`data`)) {
        self$`data` <- ApiClient$new()$deserializeObj(this_object$`data`, "array[Query]", loadNamespace("openapi"))
      }
      self
    },
    ...
    #' @return QueryList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to QueryList and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of QueryList
    toString = function() {
      self$toJSONString()
    },
...
# QueryList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })

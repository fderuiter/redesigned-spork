...
> VisitList listVisits(studyKey, opts)
...
let apiInstance = new MednetEdcApi.VisitsApi();
...
apiInstance.listVisits(studyKey, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {

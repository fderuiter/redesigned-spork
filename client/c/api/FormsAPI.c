...
//
form_list_t*
FormsAPI_listForms(apiClient_t *apiClient, char *studyKey, int *page, int *size, char *sort, char *filter)
{
    ...
    // create the path
    char *localVarPath = strdup("/studies/{studyKey}/forms");
    ...
    form_list_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *FormsAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = form_list_parseFromJSON(FormsAPIlocalVarJSON);
        cJSON_Delete(FormsAPIlocalVarJSON);

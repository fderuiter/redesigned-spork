...
VisitsAPI_listVisits(apiClient_t *apiClient, char *studyKey, int *page, int *size, char *sort, char *filter)
{
    ...
    // create the path
    char *localVarPath = strdup("/studies/{studyKey}/visits");
    ...
    visit_list_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *VisitsAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = visit_list_parseFromJSON(VisitsAPIlocalVarJSON);
        cJSON_Delete(VisitsAPIlocalVarJSON);

...
SubjectsAPI_listSubjects(apiClient_t *apiClient, char *studyKey, int *page, int *size, char *sort, char *filter)
{
    ...
    // create the path
    char *localVarPath = strdup("/studies/{studyKey}/subjects");
    ...
    subject_list_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *SubjectsAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = subject_list_parseFromJSON(SubjectsAPIlocalVarJSON);
        cJSON_Delete(SubjectsAPIlocalVarJSON);

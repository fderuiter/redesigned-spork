...
QueriesAPI_listQueries(apiClient_t *apiClient, char *studyKey, int *page, int *size, char *sort, char *filter)
{
    ...
    // create the path
    char *localVarPath = strdup("/studies/{studyKey}/queries");
    ...
    query_list_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *QueriesAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = query_list_parseFromJSON(QueriesAPIlocalVarJSON);
        cJSON_Delete(QueriesAPIlocalVarJSON);

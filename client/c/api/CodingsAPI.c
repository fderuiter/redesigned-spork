...
CodingsAPI_listCodings(apiClient_t *apiClient, char *studyKey, int *page, int *size, char *sort, char *filter)
{
    ...
    // create the path
    char *localVarPath = strdup("/studies/{studyKey}/codings");
    ...
    coding_list_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *CodingsAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = coding_list_parseFromJSON(CodingsAPIlocalVarJSON);
        cJSON_Delete(CodingsAPIlocalVarJSON);

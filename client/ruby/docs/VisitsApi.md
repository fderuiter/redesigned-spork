...
> <VisitList> list_visits(study_key, opts)
...
begin
  # List visits (subject visit instances) in a study
  result = api_instance.list_visits(study_key, opts)
  p result
rescue OpenapiClient::ApiError => e
  puts "Error when calling VisitsApi->list_visits: #{e}"
end
```

#### Using the list_visits_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<VisitList>, Integer, Hash)> list_visits_with_http_info(study_key, opts)

```ruby
begin
  # List visits (subject visit instances) in a study
  data, status_code, headers = api_instance.list_visits_with_http_info(study_key, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <VisitList>
rescue OpenapiClient::ApiError => e
  puts "Error when calling VisitsApi->list_visits_with_http_info: #{e}"
end
```

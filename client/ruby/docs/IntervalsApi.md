...
> <IntervalList> list_intervals(study_key, opts)
...
begin
  # List intervals (visit definitions) in a study
  result = api_instance.list_intervals(study_key, opts)
  p result
rescue OpenapiClient::ApiError => e
  puts "Error when calling IntervalsApi->list_intervals: #{e}"
end
```

#### Using the list_intervals_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<IntervalList>, Integer, Hash)> list_intervals_with_http_info(study_key, opts)

```ruby
begin
  # List intervals (visit definitions) in a study
  data, status_code, headers = api_instance.list_intervals_with_http_info(study_key, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <IntervalList>
rescue OpenapiClient::ApiError => e
  puts "Error when calling IntervalsApi->list_intervals_with_http_info: #{e}"
end
```

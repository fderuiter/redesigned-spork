...
> <FormList> list_forms(study_key, opts)
...
begin
  # List forms in a study
  result = api_instance.list_forms(study_key, opts)
  p result
rescue OpenapiClient::ApiError => e
  puts "Error when calling FormsApi->list_forms: #{e}"
end
```

#### Using the list_forms_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<FormList>, Integer, Hash)> list_forms_with_http_info(study_key, opts)

```ruby
begin
  # List forms in a study
  data, status_code, headers = api_instance.list_forms_with_http_info(study_key, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <FormList>
rescue OpenapiClient::ApiError => e
  puts "Error when calling FormsApi->list_forms_with_http_info: #{e}"
end
```

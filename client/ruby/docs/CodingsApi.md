...
> <CodingList> list_codings(study_key, opts)
...
begin
  # List coding activities in a study
  result = api_instance.list_codings(study_key, opts)
  p result
rescue OpenapiClient::ApiError => e
  puts "Error when calling CodingsApi->list_codings: #{e}"
end
```

#### Using the list_codings_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<CodingList>, Integer, Hash)> list_codings_with_http_info(study_key, opts)

```ruby
begin
  # List coding activities in a study
  data, status_code, headers = api_instance.list_codings_with_http_info(study_key, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <CodingList>
rescue OpenapiClient::ApiError => e
  puts "Error when calling CodingsApi->list_codings_with_http_info: #{e}"
end
```

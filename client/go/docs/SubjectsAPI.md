...
> SubjectList ListSubjects(ctx, studyKey).Page(page).Size(size).Sort(sort).Filter(filter).Execute()
...
func main() {
	...
	resp, r, err := apiClient.SubjectsAPI.ListSubjects(context.Background(), studyKey).Page(page).Size(size).Sort(sort).Filter(filter).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `SubjectsAPI.ListSubjects``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `ListSubjects`: SubjectList

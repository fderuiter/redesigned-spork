...
SiteList <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new SiteList class.
    #'
    ...
    #'
    #' @return SiteList as a base R list.
    #' @examples
    #' # convert array of SiteList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert SiteList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      SiteListObject <- list()
      if (!is.null(self$`metadata`)) {
        SiteListObject[["metadata"]] <-
          self$`metadata`$toSimpleType()
      }
      if (!is.null(self$`pagination`)) {
        SiteListObject[["pagination"]] <-
    ...
    #' @description
    #' Deserialize JSON string into an instance of SiteList
    #'
    #' @param input_json the JSON input
    #' @return the instance of SiteList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`data`)) {
        self$`data` <- ApiClient$new()$deserializeObj(this_object$`data`, "array[Site]", loadNamespace("openapi"))
      }
      self
    },
    ...
    #' @return SiteList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to SiteList and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of SiteList
    toString = function() {
      self$toJSONString()
    },
...
# SiteList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })

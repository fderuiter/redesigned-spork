...
static inline_object_1_t *inline_object_1_create_internal(
    metadata_t *metadata
    ) {
    ...
    inline_object_1_local_var->metadata = metadata;

    inline_object_1_local_var->_library_owned = 1;
...
__attribute__((deprecated)) inline_object_1_t *inline_object_1_create(
    ...
    return inline_object_1_create_internal (
        metadata
        );
}
...
void inline_object_1_free(inline_object_1_t *inline_object_1) {
    ...
    if(inline_object_1->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "inline_object_1_free");
        return ;
    }
    listEntry_t *listEntry;
    if (inline_object_1->metadata) {
        metadata_free(inline_object_1->metadata);
        inline_object_1->metadata = NULL;
    }
...
cJSON *inline_object_1_convertToJSON(inline_object_1_t *inline_object_1) {
    ...
    // inline_object_1->metadata
    if(inline_object_1->metadata) {
    cJSON *metadata_local_JSON = metadata_convertToJSON(inline_object_1->metadata);
...
inline_object_1_t *inline_object_1_parseFromJSON(cJSON *inline_object_1JSON){
    ...
    // define the local variable for inline_object_1->metadata
    metadata_t *metadata_local_nonprim = NULL;

    // inline_object_1->metadata
    cJSON *metadata = cJSON_GetObjectItemCaseSensitive(inline_object_1JSON, "metadata");
    ...
    inline_object_1_local_var = inline_object_1_create_internal (

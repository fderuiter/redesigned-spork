...
apiClient_t *apiClient_create() {
    ...
    apiClient->basePath = strdup("https://edc.prod.imednetapi.com/api/v1/edc");
    ...
    apiClient->apiKeys_securityKeyAuth = NULL;
...
, list_t *apiKeys_securityKeyAuth
) {
    apiClient_t *apiClient = malloc(sizeof(apiClient_t));
    if(basePath){
    ...
    if(apiKeys_securityKeyAuth!= NULL) {
        apiClient->apiKeys_securityKeyAuth = list_createList();
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiKeys_securityKeyAuth) {
            keyValuePair_t *pair = listEntry->data;
            keyValuePair_t *pairDup = keyValuePair_create(strdup(pair->key), strdup(pair->value));
            list_addElement(apiClient->apiKeys_securityKeyAuth, pairDup);
        }
    }else{
        apiClient->apiKeys_securityKeyAuth = NULL;
    }
...
void apiClient_free(apiClient_t *apiClient) {
    ...
    if(apiClient->apiKeys_securityKeyAuth) {
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiClient->apiKeys_securityKeyAuth) {
            keyValuePair_t *pair = listEntry->data;
            if(pair->key){
                free(pair->key);
            }
        ...
        list_freeList(apiClient->apiKeys_securityKeyAuth);
    }
    free(apiClient);
}
...
void apiClient_invoke(apiClient_t    *apiClient,
    ...
    if(handle) {
        ...
        // this would only be generated for apiKey authentication
        if (apiClient->apiKeys_securityKeyAuth != NULL)
        {
        list_ForEach(listEntry, apiClient->apiKeys_securityKeyAuth) {
        keyValuePair_t *apiKey = listEntry->data;
        if((apiKey->key != NULL) &&
